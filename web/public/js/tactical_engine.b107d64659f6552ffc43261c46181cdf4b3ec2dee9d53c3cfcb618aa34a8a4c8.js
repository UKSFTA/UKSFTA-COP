document.addEventListener('DOMContentLoaded', () => {
    ...
    const getSymbolLabel = (sym) => {
        const lookup = {
            'b_inf': 'INF', 'b_motor_inf': 'MOT INF', 'b_mech_inf': 'MECH INF',
            'b_armor': 'ARMD', 'b_recon': 'RECON', 'b_hq': 'HQ',
            'b_art': 'ARTY', 'b_mor': 'MOR', 'b_med': 'MED',
            'b_eng': 'ENGR', 'b_air': 'AVN', 'b_uav': 'UAV',
            'b_log': 'LOG', 'b_maint': 'ESME'
        };
    ...
    function initMap(theatreKey, isRemoteSync = false) {
        ...
        // Fit map to world bounds on initial load to ensure it fills the container
    ...
    function placeMarkerAt(latlng) {
        ...
        applyTacticalStyle(layer);
        drawnItems.addLayer(layer);
        log(`PLOTTED: ${getSymbolLabel(selectedSymbol)} AT ${formatMGRS(latlng.lng, latlng.lat)}`);
    }

    function applyTacticalStyle(layer) {
    ...
    window.toggleMeasure = () => {
        ...
        if (btn) {
            btn.innerText = isMeasuring ? "MEASURING (2-PT)" : "RANGEFINDER";
            btn.className = isMeasuring ? "govuk-button govuk-button--warning animate-pulse" : "govuk-button govuk-button--secondary";
        }
    ...
    function addMeasurePoint(latlng) {
        ...
        L.circleMarker(latlng, { radius: 4, color: '#003078', fillOpacity: 1 }).addTo(measureLayer);
        
        if (measurePoints.length === 2) {
            updateMeasureLine(latlng);
            // End after 2 points for a "Rangefinder" point-to-point feel
            // We keep it active so they can see the distance, then next click starts new one.
        }
    }
    ...
    function updateMeasureLine(currentLatLng) {
        ...
        const line = L.polyline(pts, { color: '#003078', dashArray: '5, 10', weight: 2 }).addTo(measureLayer);
        
        const totalDist = calculateTotalDistance(currentLatLng);
        line.bindTooltip(`${totalDist.toFixed(0)}m`, { sticky: true, permanent: true, className: 'leaflet-tooltip-tactical' }).openTooltip();
    }
